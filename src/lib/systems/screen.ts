...
export class Screen extends System {
    ...
    glyph() {
        ...
        if (this.advanced && this.area) {
            ...
            let screenLevelComponents =
                this.level === 2 || this.level === undefined
                    ? `${screenLevel1}${screenLevel2}`
                    : `${screenLevel1}`;
            let id = `screenAA${lvl}`;
            ...
            return {
                id,
                svg: `<symbol id="${id}" viewBox="280 80 400 400"><g><rect x="413" y="213" transform="matrix(0.7071 -0.7071 0.7071 0.7071 -57.4011 421.4214)" width="134" height="134"/>${screenLevelComponents}</g></symbol>`,
                height: 1,
                width: 1,
            };
        } else if (this.advanced) {
            ...
            return {
                id,
                svg: `<symbol id="${id}" viewBox="250 45 460 460"><g><rect x="387.3" y="280.8" transform="matrix(0.7071 0.7071 -0.7071 0.7071 404.6839 -230.0195)" width="185.5" height="185.5"/>${screenLevelComponents}</g></symbol>`,
                height: 1,
                width: 1,
            };
        } else if (this.area) {
            ...
            return {
                id,
                svg: `<symbol id="${id}" viewBox="302.5 102.5 355 355"><g><circle cx="480" cy="280" r="63"/>${screenLevelComponents}</g></symbol>`,
                width: 1,
                height: 1,
            };
        } else {
            ...
            return {
                id,
                svg: `<symbol id="${id}" viewBox="250 40 460 460"><g><circle cx="480" cy="366.8" r="131.8"/>${screenLevelComponents}</g></symbol>`,
                height: 1,
                width: 1,
            };
        }
