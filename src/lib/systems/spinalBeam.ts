...
export class SpinalBeam extends System {
    ...
    glyph() {
        if (this.range === "short") {
            ...
            const id = this.ship.hashseed === undefined ? `spinalBeamL` : fnv.hash(`spinalBeamL`).hex();
            return {
                id,
