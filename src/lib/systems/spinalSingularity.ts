...
export class SpinalSingularity extends System {
    ...
    glyph() {
        if (this.range === "short") {
            ...
            const id = this.ship.hashseed === undefined ? `spinalSingularityL` : fnv.hash(`spinalSingularityL`).hex();
            return {
                id,
