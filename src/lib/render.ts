...
export const renderSvg = (ship: FullThrustShip, opts: RenderOpts = {}): string | undefined => {
    ...
    if ( (ship.hasOwnProperty("hull")) && (ship.hull !== undefined) ) {
        ...
        if (sysDrive !== undefined) {
            svgDrive = sysLib.getSystem(sysDrive, ship)!.glyph();
        }
        ...
        svg += `<defs>`;
        svg += `<style type="text/css"><![CDATA[ @import url('https://fonts.googleapis.com/css2?family=Zen+Dots&family=Fira+Sans&display=swap'); svg {font-family: "Fira Sans";} .futureFont { font-family: "Zen Dots" } .svgInvert { filter: invert(1); } ]]></style>`;
        ...
        svg += `<rect x="0" y="${(totalRows - 3) * cellsize}" width="${pxWidth}" height="${cellsize * 3}" stroke="none" fill="black"/>`;
        ...
        if (magazines.length > 0) {
            for (const mag of magazines) {
                ...
                for (let i = 0; i < mag.capacity; i++) {
                    ...
                    svg += `<use href="#svg_${glyph.id}" x="${((realCol * 2) * cellsize) + buff.xOffset}" y="${((currRow + (realRow * 2)) * cellsize) + buff.yOffset}" width="${buff.width}" height="${buff.height}" />`;
                }
        ...
        if (turrets.length > 0) {
            for (const turret of turrets) {
                ...
                svg += `<use id="${turret.uid}" href="#svg_${turret.glyph().id}" x="${buff.xOffset}" y="${(currRow * cellsize) + buff.yOffset}" width="${buff.width}" height="${buff.height}" />`;
                ...
                for (let i = 0; i < turret.weapons.length; i++) {
                    ...
                    const buff = buffInSquare(glyph, cellsize * 2, false);
                    svg += `<use id="${weapon.uid}" href="#svg_${glyph.id}" x="${((realCol * 2) * cellsize) + buff.xOffset}" y="${((currRow + (realRow * 2)) * cellsize) + buff.yOffset}" width="${buff.width}" height="${buff.height}" />`;
                }
        ...
        if (compact) {
            ...
            if (svgDrive !== undefined) {
                svg += `<use id="_drive" href="#svg_${svgDrive.id}" x="${pxWidth - (cellsize * 3)}" y="${(hullStart + 2) * cellsize}" width="${cellsize * 2}" height="${cellsize * 2}" />`;
            }
            svg += `<use id="_core" href="#svg_${svgCore.id}" x="${pxWidth * 0.05}" y="${(currRow * cellsize) + ((cellsize * 3) * 0.05)}" width="${pxWidth * 0.9}" height="${(cellsize * 3) * 0.9}" class="svgInvert" />`;
        } else {
            let svgCombined = "";
            let startX = 0;
            let groupWidth = 9;
            ...
            svg += `<use href="#_internalLinearCombined" x="0" y="${currRow * cellsize}" height="${cellsize * 3}" width="${pxWidth}" class="svgInvert" />`;
        }
