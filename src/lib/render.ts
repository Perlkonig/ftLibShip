...
export const renderSvg = (ship: FullThrustShip, opts: RenderOpts = {}): string | undefined => {
    ...
    if ( (ship.hasOwnProperty("hull")) && (ship.hull !== undefined) ) {
        ...
        svg += `<defs>`;
        svg += `<style type="text/css"><![CDATA[ @import url('https://fonts.googleapis.com/css2?family=Zen+Dots&display=swap'); .futureFont { font-family: "Zen Dots" } .svgInvert { filter: invert(1); } ]]></style>`;
